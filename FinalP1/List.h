...
public:
    ...
    {
        Node lCurrent = fTail;
        fTail.reset(); // reduce reference count of last element

        while (lCurrent)
        {
            auto prevNode = lCurrent->fPrevious.lock(); // get shared_ptr to previous node
            if (prevNode)
            {
                prevNode->fNext.reset(); // cut the link to current node
            }
            else
            {
                fHead.reset(); // if there is no previous node, reset fHead
            }

            lCurrent = prevNode; // move to the previous node
        }
    }
